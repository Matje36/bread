...
# ---------------------------------------------------------
# MUSIC SYSTEM CONSTANTS (Definitieve Formaatinstelling)
# ---------------------------------------------------------

# FFMPEG_OPTIONS blijft hetzelfde
FFMPEG_OPTIONS = {
    'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
    'options': '-vn' 
}

# YDL_OPTIONS wordt aangepast
YDL_OPTIONS = {
    # HIER IS DE VEREENVOUDIGING: gewoon de beste audio, ongeacht de container.
...
class MusicSystem(commands.Cog):
    # 1. Define Groups as Class Attributes FIRST
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        def after_playing(error):
            """Callback triggered when current song finishes or errors."""
            if error:
                print(f"❌ Playback Error/Crash detected: {error}")
                coro_cleanup = self.force_cleanup(interaction.guild)
        ...
        try:
            # --- DEBUG 2: Check URL just before FFmpeg is called ---
            print(f"DEBUG-PLAY: Starting FFmpeg for {title}. URL starts with: {url[:50]}...")
            # --------------------------------------------------------
            ...
            print(f"CRITICAL ERROR playing song: {e}. Check FFmpeg logs for stream access issues.")
            asyncio.run_coroutine_threadsafe(
                interaction.channel.send(f"⚠️ Error playing **{title}**. Skipped. (Check logs for stream URL issue)"), 
                self.bot.loop
            )
            coro = self.play_next(interaction)
    ...
    async def force_cleanup(self, guild):
        """Helper function to forcefully disconnect and clear state after a crash."""
        voice_client = guild.voice_client
        if voice_client:
            await voice_client.disconnect(force=True)
        self.queues.pop(guild.id, None)
        self.current_song.pop(guild.id, None)
        print(f"INFO: Forcefully cleaned up state for guild {guild.id}.")
    ...
    async def call_join(self, interaction: discord.Interaction):
        ...
        if voice_client:
            if voice_client.channel.id != channel.id:
                ...
                await interaction.response.send_message("✅ I am already here!", ephemeral=True)
        else:
    ...
    @sound_group.command(name="play", description="Play audio from a YouTube URL or search query (adds to queue).")
    ...
    async def sound_play(self, interaction: discord.Interaction, url: str):
        ...
        try:
            with yt_dlp.YoutubeDL(YDL_OPTIONS) as ydl:
                ...
                # --- DEBUG 1: Check the fetched URL from yt-dlp ---
                print(f"DEBUG-YTDLP: Fetched Stream URL (first 50 chars): {stream_url[:50] if stream_url else 'NONE'}")
                # --------------------------------------------------
                
                if not stream_url:
                     return await interaction.followup.send("❌ Could not find a valid stream URL for the requested song.")
            ...
            print(f"❌ YTDLP Extraction Error: {error_details}")
            return await interaction.followup.send(f"❌ Error fetching song info. Logged details. (Check **cookies.txt** or URL validity)")
    ...
    @sound_group.command(name="skip", description="Skips the currently playing track.")
    ...
    async def sound_stop(self, interaction: discord.Interaction):
        voice_client = interaction.guild.voice_client
        if voice_client and (voice_client.is_playing() or len(self.get_queue(interaction.guild.id)) > 0):
            self.queues.pop(interaction.guild.id, None)
            if voice_client.is_playing():
                voice_client.stop()
            await interaction.response.send_message("🛑 Music stopped and queue cleared.", ephemeral=True)
        else:
            await interaction.response.send_message("❌ Nothing is currently active.", ephemeral=True)
    ...
    async def sound_queue(self, interaction: discord.Interaction):
        ...
        if current:
             desc += f"**▶️ Now Playing:** {current['title']} (by {current['requester']})\n\n**Up Next:**\n"
    ...
    @sound_group.command(name="refresh", description="Hard reset: Kicks bot, clears queue, and resets module state.")
    async def sound_refresh(self, interaction: discord.Interaction):
        ...
        if voice_client:
            if voice_client.is_playing():
                voice_client.stop()
            await voice_client.disconnect(force=True)
            await interaction.response.send_message("🔄 **Refresh:** Audio system forcefully reset, queue cleared, and bot disconnected.", ephemeral=True)
        else:
            await interaction.response.send_message("🔄 **Refresh:** Queue cleared (bot was not connected).", ephemeral=True)
...
from r6_status import fetch_r6_status
import discord
from discord import app_commands
...
@bot.tree.command(
    name="r6status",
    description="Get the current Rainbow Six Siege server status.",
    guild=discord.Object(id=GUILD_ID)
)
async def r6status(interaction: discord.Interaction):
    await interaction.response.defer()
    
    status = await fetch_r6_status()
    
    if "error" in status:
        await interaction.followup.send(f"❌ Error fetching R6 server status: {status['error']}")
        return
    ...
    for platform, info in status.items():
        ...
        if info.get("maintenance"):
            emoji = "🟡"
        embed.add_field(name=platform, value=f"{emoji} {'Online' if info['online'] else 'Offline / Maintenance'}", inline=True)
