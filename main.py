...
class VoiceTracker(commands.Cog):
    ...
    async def generate_gta_card(self, user: discord.User, data):
        # Canvas Settings
        WIDTH, HEIGHT = 900, 1500  # Hoogte is 1500 om alles te passen
        ...
        # Fonts (GEFIXED EN GROTER)
        font_header_bold = load_font("arialbd.ttf", 150) # Grotere header
        font_label = load_font("arialbd.ttf", 80) # Grotere label
        font_value = load_font("arial.ttf", 80) # Grotere waarde
        ...
        # Gradient Fade for Banner (Dark Overlay)
        overlay = Image.new("RGBA", (WIDTH, banner_h), (0, 0, 0, 100))
        banner = Image.alpha_composite(banner, overlay)
        
        # Paste banner onto card (GEEN MASK, OM TOT AAN DE RAND TE KOMEN)
        ...
        # Bereken breedte met nieuwe lettertype
        t_bbox = draw.textbbox((0,0), name_text, font=font_header_bold)
        t_w = t_bbox[2] - t_bbox[0]
        name_x = (WIDTH - t_w) // 2
        name_y = av_y + av_size + 40 # Verhoogde Y positie
        ...
        # Aangepaste maten om binnen 1500px te passen
        start_y = name_y + 200 
        item_height = 90 # Aangepast naar 90
        margin_x = 50
        
        for i, (label, value) in enumerate(stats_list):
            # Item spacing is nu 15px
            ...
            # Accent Bar on Left (Red)
            row_draw.rectangle([margin_x, 0, margin_x + 12, item_height], fill=(255, 0, 0))
            
            # Paste the row
            card.paste(row_layer, (0, y_pos), row_layer)
            
            # Text
            # Label (Left) - Gebruik getbbox voor correcte verticale centrering
            label_bbox = draw.textbbox((0,0), label, font=font_label)
            label_y_offset = y_pos + (item_height - (label_bbox[3] - label_bbox[1])) // 2
            draw.text((margin_x + 40, label_y_offset), label, font=font_label, fill=(220, 220, 220))
            
            # Value (Right)
            val_bbox = draw.textbbox((0,0), value, font=font_value)
            val_w = val_bbox[2] - val_bbox[0]
            val_y_offset = y_pos + (item_height - (val_bbox[3] - val_bbox[1])) // 2
            draw.text((WIDTH - margin_x - val_w - 40, val_y_offset), value, font=font_value, fill=(255, 255, 255))
