...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        try:
            ...
            stat_font = ImageFont.truetype("arial.ttf", 56)
            label_font = ImageFont.truetype("arial.ttf", 44)
            username_font = ImageFont.truetype("arial.ttf", 60)  # Increased size for name
            base_title_font = ImageFont.truetype("arial.ttf", 160)  # Much bigger base for title
        except Exception:
        ...
        def fit_font_for_width(text, base_font, max_width, min_size=28):
        ...
        banner = banner.filter(ImageFilter.GaussianBlur(radius=10)) # bluuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuur 18

        # dark overlay
        dark = Image.new("RGBA", (WIDTH, banner_h), (10, 10, 15, 100)) # kanusssss darknessss 160
        ...
        username_text = user.name.upper().bold() # bold weggg
        # Try to use bold font for username
        try:
            username_bold_font = ImageFont.truetype("arialbd.ttf", 120)  # Bigger and bold hieeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeer 60
        except:
        ...
        uname_y = avatar_y + AV_SIZE + BORDER * 2 + 10
        ...
        stats_top = uname_y + (username_bold_font.size if hasattr(username_bold_font, 'size') else 60) + 18
        y = stats_top
        spacing = 86
        box_padding = 28
        box_left = 40
        ...
        try:
            label_big = ImageFont.truetype("arialbd.ttf", 46)  # Bigger and bold
            value_big = ImageFont.truetype("arialbd.ttf", 100)  # Bigger and bold  kaksalikslakalkslaklsaklasklakslkalskalkslaklkdddddddddddddddddddddddddddddddddddddddddddd 64
        except Exception:
            try:
                label_big = ImageFont.truetype("arial.ttf", 46)  # Fallback to regular
                value_big = ImageFont.truetype("arial.ttf", 100)# Bigger and bold  kaksalikslakalkslaklsaklasklakslkalskalkslaklk 64
            except Exception:
        ...
        def stat(label, value):
            ...
            # left label (orange-ish)
            lbl_text = f"{label}:"
            lbl_x = box_left + box_padding
            lbl_y = box_top + (spacing - 12 - (label_big.size if hasattr(label_big, "size") else 40)) // 2
            ...
            val_y = box_top + (spacing - 12 - val_h) // 2
