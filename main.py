...
#event lisent 
    ...
    async def on_voice_state_update(self, member, before, after):
        print(f"DEBUG: {member.name} | before: {before.channel} | after: {after.channel}")  # ADD THIS
        ...
        if before.self_deaf != after.self_deaf and after.self_deaf:  # FIXED
            stats[user_id]["deaf_count"] += 1

        await save_stats(stats)

    # Remove the broken on_speaking listener — it doesn't exist in discord.py
    # If you want talking time, use a voice client with audio processing (complex)
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        # local imports used only here to avoid editing top imports
        from PIL import ImageFilter, ImageOps

        # fonts (load with fallback) — larger sizes so text is big and readable
        try:
            # title font will be resized to fit width below
            stat_font = ImageFont.truetype("arial.ttf", 100)  # Increased from 80
            label_font = ImageFont.truetype("arial.ttf", 110)  # Increased from 90
            username_font = ImageFont.truetype("arial.ttf", 100)  # Increased from 80
            base_title_font = ImageFont.truetype("arial.ttf", 110)  # Increased from 90
        except Exception:
            stat_font = ImageFont.load_default()
            label_font = ImageFont.load_default()
            username_font = ImageFont.load_default()
            base_title_font = ImageFont.load_default()

        # Background: FULL BLACK as requested
        draw.rectangle([0, 0, WIDTH, HEIGHT], fill=(0, 0, 0))

        # Thin top bar (make it subtle/thinner)
        BAR_H = 54  # Slightly increased
        bar_y0 = 0
        bar_y1 = BAR_H
        draw.rectangle([0, bar_y0, WIDTH, bar_y1], fill=(0, 0, 0, 230))  # Black bar

        # Title centered and very large, auto-scale to fit width with small padding
        title_text = "VOICE STATS"
        padding_x = 40  # Increased padding

        # auto-scale title font size to fit across the bar
        def fit_font_for_width(text, base_font, max_width, min_size=100):  # Increased min_size
            size = getattr(base_font, "size", 160) 
            # fallback simple loop reducing size until fits
            for s in range(size, min_size - 1, -2):
                try:
                    if hasattr(base_font, "path"):
                        f = ImageFont.truetype(base_font.path, s)
                    else:
                ...
                w = bbox[2] - bbox[0]
                h = bbox[3] - bbox[1]
                if w <= max_width - padding_x * 2:
                    return f, w, h
            # fallback that returns (font, width, height) — never returns a nested tuple
            bbox = draw.textbbox((0, 0), text, font=base_font)
            w = bbox[2] - bbox[0]
            h = bbox[3] - bbox[1]
            return base_font, w, h

        title_font, title_w, title_h = fit_font_for_width(title_text, base_title_font, WIDTH)
        title_h = draw.textbbox((0, 0), title_text, font=title_font)[3] - draw.textbbox((0, 0), title_text, font=title_font)[1]
        title_x = (WIDTH - title_w) // 2
        title_y = (BAR_H - title_h) // 2

        # Title stays black as requested - no shadow, just black text
        draw.text((title_x, title_y), title_text, font=title_font, fill=(0, 0, 0))

        # Read avatar bytes and prepare banner behind avatar
        avatar_bytes = await user.display_avatar.read()
        avatar = Image.open(BytesIO(avatar_bytes)).convert("RGBA")

        # Banner created from avatar: wide, blurred, darkened, nicely fitted behind avatar
        banner_h = 260  # Increased height
        banner = avatar.copy().resize((WIDTH, banner_h))
        banner = banner.filter(ImageFilter.GaussianBlur(radius=2)) # bluuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuur 18

        # dark overlay
        dark = Image.new("RGBA", (WIDTH, banner_h), (10, 10, 15, 40)) # kanusssss darknessss 160
        banner = Image.alpha_composite(banner, dark)

        # paste banner slightly below the bar
        banner_y = BAR_H + 20  # Increased spacing
        card.paste(banner, (0, banner_y), banner)

        # Avatar smaller and centered, with thin dark red border (stays the same)
        AV_SIZE = 190  # Increased size
        ...
        # create border (thin, darker red) - stays the same
        BORDER = 7  # Slightly increased border
        border_color = (90, 8, 8, 255)  # very dark red
        avatar_border = Image.new("RGBA", (AV_SIZE + BORDER * 2, AV_SIZE + BORDER * 2), (0, 0, 0, 0))
        bd_draw = ImageDraw.Draw(avatar_border)
        bd_draw.ellipse((0, 0, AV_SIZE + BORDER * 2 - 1, AV_SIZE + BORDER * 2 - 1), fill=border_color)
        avatar_border.paste(avatar_small, (BORDER, BORDER), mask)

        # position avatar centered horizontally, overlapping banner nicely
        avatar_x = (WIDTH - (AV_SIZE + BORDER * 2)) // 2
        avatar_y = banner_y + (banner_h - (AV_SIZE + BORDER * 2)) // 2
        card.paste(avatar_border, (avatar_x, avatar_y), avatar_border)

        # Username centered under avatar - BIGGER, FATTER, RED
        username_text = user.name.upper()
        # Try to use bold font for username
        try:
            username_bold_font = ImageFont.truetype("arialbd.ttf", 100)  # Increased size
        except:
            username_bold_font = username_font  # Fallback to regular if bold not available
        ...
        uname_y = avatar_y + AV_SIZE + BORDER * 2 + 15  # Increased spacing
        
        # Red username with dark shadow for better readability
        draw.text((uname_x + 2, uname_y + 2), username_text, font=username_bold_font, fill=(20, 20, 20))
        draw.text((uname_x, uname_y), username_text, font=username_bold_font, fill=(200, 30, 30))

        # Stats area: make boxes with black inside, bigger text, and red glow
        stats_top = uname_y + (username_bold_font.size if hasattr(username_bold_font, 'size') else 100) + 22  # Increased spacing
        y = stats_top
        spacing = 120  # Increased spacing
        box_padding = 35  # Increased padding
        box_left = 50  # Increased margin
        box_right = WIDTH - 50  # Increased margin

        # fonts for labels/values - bigger and try to use different font
        try:
            label_big = ImageFont.truetype("arialbd.ttf", 90)  # Bigger and bold
            value_big = ImageFont.truetype("arialbd.ttf", 90)  # Bigger and bold
        except Exception:
            try:
                label_big = ImageFont.truetype("arial.ttf", 90)  # Fallback to regular
                value_big = ImageFont.truetype("arial.ttf", 90)  # Bigger and bold
            except Exception:
                label_big = label_font
                value_big = stat_font

        def stat(label, value):
            nonlocal y
            # background box that spans across image - BLACK INSIDE as requested
            box_top = y
            box_bottom = y + spacing - 15  # Adjusted for new spacing
            
            # Create red glow effect
            glow_layer = Image.new("RGBA", card.size, (0, 0, 0, 0))
            glow_draw = ImageDraw.Draw(glow_layer)
            
            # Draw multiple blurred rectangles for glow effect
            ...
            card.paste(Image.alpha_composite(card, glow_layer), (0, 0))
            
            # Main box with black fill and red outline
            draw.rectangle([box_left, box_top, box_right, box_bottom], 
                         fill=(0, 0, 0, 255),  # Solid black inside
                         outline=(200, 50, 40, 255),  # Red outline
                         width=3)

            # label (total vctieme, etc) on left side
            lbl_text = f"{label}:"
            lbl_x = box_left + box_padding
            lbl_y = box_top + (spacing - 15 - (label_big.size if hasattr(label_big, "size") else 40)) // 2
            draw.text((lbl_x, lbl_y), lbl_text, font=label_big, fill=(255, 255, 255))

            # right value (green) aligned to right inside box
            val_text = str(value)
            val_bbox = draw.textbbox((0, 0), val_text, font=value_big)
            val_w = val_bbox[2] - val_bbox[0]
            val_h = val_bbox[3] - val_bbox[1]
            val_x = box_right - box_padding - val_w
            val_y = box_top + (spacing - 15 - val_h) // 2
    ...
    async def vcstats(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        try:
            ...
            print(f"DEBUG /vcstats: Looking for uid={uid}, available keys: {list(stats.keys())}")  # ADD THIS
            print(f"DEBUG /vcstats: Full stats data: {stats}")  # ADD THIS
