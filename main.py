...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        banner = banner.filter(ImageFilter.GaussianBlur(radius=18))

        # dark overlay
        dark = Image.new("RGBA", (WIDTH, banner_h), (10, 10, 15, 160))
        ...
        try:
            username_bold_font = ImageFont.truetype("arialbd.ttf", 60)  # Bigger and bold
        except:
        ...
        try:
            ...
            value_big = ImageFont.truetype("arialbd.ttf", 64)  # Bigger and bold
        except Exception:
            try:
                label_big = ImageFont.truetype("arial.ttf", 46)  # Fallback to regular
                value_big = ImageFont.truetype("arial.ttf", 64)
            except Exception:
        ...
        def stat(label, value):
            ...
            draw.text((lbl_x, lbl_y), lbl_text, font=label_big, fill=(255, 150, 0))
...
async def main():
    # await init_db()  # DELETE THIS LINE
    await bot.add_cog(VoiceTracker(bot))
    async with bot:
