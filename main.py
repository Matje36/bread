...
class VoiceTracker(commands.Cog):
    ...
    # ------------------------------------------------------------------
    #  IMAGE GENERATOR (OPTIMIZED DISCORD CARD V2)
    # ------------------------------------------------------------------
    async def generate_gta_card(self, user: discord.User, data):

        # Adjusted dimensions for better proportions (900x1000)
        WIDTH = 900
        HEIGHT = 1000 
        card = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255)) # Black background
        draw = ImageDraw.Draw(card)

        # Fonts (Significantly reduced sizes)
        try:
            # Stats text font sizes
            title_font = ImageFont.truetype("arialbd.ttf", 45)  # Title at top
            label_big = ImageFont.truetype("arialbd.ttf", 40)   # White Labels
            value_big = ImageFont.truetype("arialbd.ttf", 40)   # Green Values
            username_bold_font = ImageFont.truetype("arialbd.ttf", 65) # Red Username
        except Exception:
        ...
        banner_h = 250 # Adjusted banner height
        ...
        # Position avatar centered horizontally
        avatar_full_size = AV_SIZE + BORDER * 2
        avatar_x = (WIDTH - avatar_full_size) // 2
        avatar_y = banner_h - (avatar_full_size // 2) - 10 # Overlap bottom of banner
        card.paste(avatar_border, (avatar_x, avatar_y), avatar_border)

        # --- Username centered under avatar (smaller font) ---
        ...
        uname_y = avatar_y + avatar_full_size + 15 # Just below the avatar border
        
        # Red username with dark shadow
        draw.text((uname_x + 2, uname_y + 2), username_text, font=username_bold_font, fill=(20, 20, 20)) # Shadow
        draw.text((uname_x, uname_y), username_text, font=username_bold_font, fill=(200, 30, 30)) # Red

        # --- Stats area: Boxes (Adjusted spacing and sizes) ---
        stats_top = uname_y + 100 # Starting position for the first box
        y = stats_top
        spacing = 70 # Vertical space between boxes (Box height + 15)
        box_padding = 20 # Padding inside the box (text to box border)
        box_left = 40 # Margin on left
        box_right = WIDTH - 40 # Margin on right
        box_height = 55 # Fixed height for the boxes
        ...
        if y > HEIGHT:
            # If stats overflow, we need a taller canvas (this shouldn't happen with 1000px)
            print(f"Warning: Stats boxes exceeded image height. Final Y: {y}")
            # In a real scenario, you might want to recreate the image with HEIGHT = y + 40
