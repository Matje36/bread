...
class StopwatchView(discord.ui.View):
    ...
    async def update_loop(self):
        """Achtergrondtaak die elke seconde de embed update."""
        while self.running:
    ...
    async def start_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.user:
            return await interaction.response.send_message(
                "❌ Dit is niet jouw stopwatch.",
                ephemeral=True
            )

        if self.running:
            return await interaction.response.send_message(
                "⏱️ Stopwatch draait al.",
                ephemeral=True
            )
    ...
    async def stop_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="⏱️ Stopwatch (Gestopt)",
            description=f"`{h:02d}:{m:02d}:{s:02d}`",
            color=discord.Color.red()
        )

        await self.message.edit(embed=embed, view=self)
        await interaction.response.send_message("⏹️ Stopwatch gestopt!", ephemeral=True)
