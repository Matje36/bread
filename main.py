...
# YT-DLP Options (for speed and audio quality)
YTDL_OPTIONS = {
    ...
    'http_headers': { # Helps against IP blocks
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    }
}

# FFmpeg Options (for stable stream)
FFMPEG_OPTIONS = {
    'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
    'options': '-vn -loglevel quiet', # No video, audio only, quieter logs
}
...
class MusicSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # Queue structure: {guild_id: deque([url1, url2, ...])}
        self.queues = {} 
    ...
    async def play_next(self, interaction: discord.Interaction):
        """Plays the next song in the queue. Bot remains connected when queue is empty."""
        guild_id = interaction.guild_id
        queue = self.get_queue(guild_id)
        
        # Retrieve Voice Client explicitly
        vc = discord.utils.get(self.bot.voice_clients, guild=interaction.guild)

        # Check connection. If disconnected, clear queue but DO NOT try to connect again.
        if not vc or not vc.is_connected():
            if len(queue) > 0:
                self.queues[interaction.guild_id] = deque()
                await interaction.channel.send("⚠️ Bot disconnected unexpectedly. Queue cleared.")
            return

        # If queue is empty, stop execution but remain connected
        if len(queue) == 0:
            await interaction.channel.send("✅ Queue is empty. I will stay connected until you use `/call leave`.")
            return
        ...
        try:
            # 1. Fetch info (quick step)
            data = await loop.run_in_executor(None, lambda: ytdl.extract_info(url, download=False))
            song_url = data.get('url')
            title = data.get('title', 'Unknown song')
            
            # 2. Start audio stream (requires FFmpeg)
            source = discord.FFmpegPCMAudio(song_url, **FFMPEG_OPTIONS)
            
            # Callback: when song is finished, check next one
            def after_playing(error):
                if error:
                    print(f"Player error: {error}")
                # Call play_next again in the bot's loop
                fut = asyncio.run_coroutine_threadsafe(self.play_next(interaction), self.bot.loop)
                try:
                    fut.result()
                except Exception as e:
                    print(f"Error starting next song in thread: {e}")

            vc.play(source, after=after_playing)
            
            # Send update in chat
            await interaction.channel.send(f"▶️ Now playing: **{title}**")
            
        except yt_dlp.DownloadError as e:
            error_detail = str(e).split('\n')[0]
            print(f"YTDL Error: {error_detail}")
            await interaction.channel.send(f"❌ **Download Error**: Could not process the URL. Reason: `{error_detail}`", ephemeral=False)
            ...
            if "ffmpeg was not found" in error_message:
                 await interaction.channel.send(
                    f"⚠️ **FFmpeg Error** (`{type(e).__name__}`): The song failed to start. **FFmpeg was not found.** Check Railway settings (Dockerfile/Redeploy).", 
                    ephemeral=False
                )
            else:
                 await interaction.channel.send(
                    f"⚠️ **General Playback Error** (`{type(e).__name__}`): The song was moved back to the queue. Error: `{e}`", 
                    ephemeral=False
                )


    # --- GROUP: /youtube (Music Controls) ---
    youtube_group = app_commands.Group(name="youtube", description="Music commands", guild_ids=[GUILD_ID])

    @youtube_group.command(name="play", description="Play music from a URL (or add to queue)")
    async def play(self, interaction: discord.Interaction, url: str):
        ...
        if vc.is_playing() or vc.is_paused():
            ...
            await interaction.followup.send(f"🔍 Loading...")
            await self.play_next(interaction)

    @youtube_group.command(name="pause", description="Pause the music")
    ...
    @youtube_group.command(name="resume", description="Resume the music")
    ...
    @youtube_group.command(name="skip", description="Skip the current song")
    ...
    @youtube_group.command(name="queue", description="View the current queue")
    async def queue_cmd(self, interaction: discord.Interaction):
        ...
        if len(queue) > 10:
            msg += f"...and {len(queue)-10} more songs."
            
        await interaction.response.send_message(msg)

    @youtube_group.command(name="testsound", description="Tests if audio playback works (requires FFmpeg)")
    async def testsound(self, interaction: discord.Interaction):
        # ... (test sound logic remains the same)
        url = "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"
        
        if not interaction.user.voice:
            return await interaction.response.send_message("Please join a voice channel first.")
        ...
        await interaction.response.send_message("🔊 Testing audio playback...")
        
        try:
            vc.stop() 
            vc.play(discord.FFmpegPCMAudio(url, **FFMPEG_OPTIONS))
            await interaction.followup.send("✅ Test started successfully! If you hear sound, FFmpeg is correctly installed.", ephemeral=False)
        except Exception as e:
            traceback.print_exc()
            await interaction.followup.send(f"❌ **FFmpeg Error**: Could not start the audio stream. This strongly suggests missing or incorrect FFmpeg installation. Error: `{e}`", ephemeral=False)


    # --- NEW GROUP: /call (Connection Management) ---
    call_group = app_commands.Group(name="call", description="Connection control commands", guild_ids=[GUILD_ID])

    @call_group.command(name="join", description="Joins your current voice channel.")
    async def call_join(self, interaction: discord.Interaction):
        await interaction.response.defer()

        if not interaction.user.voice:
            return await interaction.followup.send("❌ You must be in a voice channel to call the bot.")
        ...
        if not vc:
            ...
            await interaction.followup.send(f"👋 Joined **{channel.name}**. I will stay connected until `/call leave`.")
        elif vc.channel == channel:
            await interaction.followup.send("✅ I am already in your voice channel.")
        else:
            await vc.move_to(channel)
            await interaction.followup.send(f"✅ Moved to **{channel.name}**.")
            
    @call_group.command(name="leave", description="Stops music, clears the queue, and disconnects.")
    async def call_leave(self, interaction: discord.Interaction):
        ...
        if vc:
            ...
            await vc.disconnect()
            await interaction.response.send_message("👋 Disconnected. Queue cleared.")
        else:
