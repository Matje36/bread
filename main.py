...
# FFmpeg options for stable reconnecting streams
FFMPEG_OPTIONS = {
...
class YTAudio(discord.PCMVolumeTransformer):
    """Wrapper to allow volume control."""
...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        # No songs left
        if not queue:
            await interaction.channel.send("✔️ Queue finished — bot will stay connected.")
            return
        ...
        try:
            ...
            await interaction.channel.send(f"❌ Error processing URL: `{e}`")
            return

        # Play audio
        try:
            source = discord.FFmpegPCMAudio(audio_url, **FFMPEG_OPTIONS)
            audio = YTAudio(source)
            ...
            await interaction.channel.send(f"❌ FFmpeg error: `{e}`")
            return
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        if vc.is_playing() or vc.is_paused():
            return await interaction.followup.send(f"➕ Added to queue:\n`{query}`")
    ...
    # /youtube resume
    # ----------------------------------------------------
    @youtube_group.command(name="resume", description="Resume paused music.")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    ...
    # /youtube queue
    # ----------------------------------------------------
    @youtube_group.command(name="queue", description="Show the current music queue.")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def queue_cmd(self, interaction: discord.Interaction):
        ...
        msg = "**🎶 Current Queue:**\n"
        for i, item in enumerate(list(queue)[:10], start=1):
            msg += f"{i}. `{item}`\n"
    ...
    # /youtube testsound
    # ----------------------------------------------------
    @youtube_group.command(name="testsound", description="Check if audio playback works.")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def testsound(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("🔊 Test sound playing — if you hear it, audio works!")
        except Exception as e:
