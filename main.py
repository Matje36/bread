...
class VoiceTracker(commands.Cog):
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        # Load fonts
        try:
            username_font = ImageFont.truetype("arialbd.ttf", 48)
            label_font = ImageFont.truetype("arial.ttf", 32)
            value_font = ImageFont.truetype("arialbd.ttf", 32)
        except Exception:
            try:
                username_font = ImageFont.truetype("Arial", 48)
                label_font = ImageFont.truetype("Arial", 32)
                value_font = ImageFont.truetype("Arial", 32)
            except Exception:
        ...
        banner = banner.filter(ImageFilter.GaussianBlur(radius=3))

        # Dark overlay for banner
        dark = Image.new("RGBA", (WIDTH, banner_h), (10, 10, 15, 120))
        banner = Image.alpha_composite(banner, dark)

        # Paste banner at top
        card.paste(banner, (0, 0), banner)

        # Avatar in center of banner
        AV_SIZE = 180
        ...
        # Position avatar in center of banner
        avatar_x = (WIDTH - (AV_SIZE + BORDER * 2)) // 2
        avatar_y = (banner_h - (AV_SIZE + BORDER * 2)) // 2
        card.paste(avatar_border, (avatar_x, avatar_y), avatar_border)

        # Username centered below avatar - larger and red
        ...
        draw.text((uname_x + 3, uname_y + 3), username_text, font=username_font, fill=(20, 20, 20))
        draw.text((uname_x, uname_y), username_text, font=username_font, fill=(200, 30, 30))

        # Stats boxes
        stats_top = uname_y + 80
        y = stats_top
        spacing = 85
        box_padding = 25
        box_left = 40
        ...
        def stat(label, value):
            ...
            lbl_text = f"{label}:"
            lbl_bbox = draw.textbbox((0, 0), lbl_text, font=label_font)
            lbl_h = lbl_bbox[3] - lbl_bbox[1]
            lbl_x = box_left + box_padding
            lbl_y = box_top + (spacing - 15 - lbl_h) // 2
            draw.text((lbl_x, lbl_y), lbl_text, font=label_font, fill=(255, 255, 255))

            # Value on right side (green)
            val_text = str(value)
            val_bbox = draw.textbbox((0, 0), val_text, font=value_font)
            val_w = val_bbox[2] - val_bbox[0]
            val_h = val_bbox[3] - val_bbox[1]
            val_x = box_right - box_padding - val_w
            val_y = box_top + (spacing - 15 - val_h) // 2
            draw.text((val_x, val_y), val_text, font=value_font, fill=(20, 220, 120))
