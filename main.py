...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        # Retrieve Voice Client explicitly (FIX for "Not connected to voice" errors)
        vc = discord.utils.get(self.bot.voice_clients, guild=interaction.guild)

        if not vc or not vc.is_connected():
            # If bot unexpectedly disconnected, clear queue and notify
            if len(queue) > 0:
        ...
        if len(queue) == 0:
            # Check if bot has been idle for a while (optional auto-disconnect logic could go here)
            return
