...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        HEIGHT = 2000  # Much larger height
        ...
        # fonts (load with fallback) — 10x bigger sizes
        try:
            stat_font = ImageFont.truetype("arial.ttf", 800)  # 10x bigger
            label_font = ImageFont.truetype("arial.ttf", 900)  # 10x bigger
            username_font = ImageFont.truetype("arial.ttf", 800)  # 10x bigger
            base_title_font = ImageFont.truetype("arial.ttf", 1100)  # 10x bigger
        except Exception:
        ...
        # Banner touches top of PNG - no top bar anymore
        # Read avatar bytes and prepare banner behind avatar
        avatar_bytes = await user.display_avatar.read()
        avatar = Image.open(BytesIO(avatar_bytes)).convert("RGBA")

        # Banner created from avatar: wide, blurred, darkened, touches top
        banner_h = 400  # Much bigger banner
        banner = avatar.copy().resize((WIDTH, banner_h))
        banner = banner.filter(ImageFilter.GaussianBlur(radius=2))

        # dark overlay
        dark = Image.new("RGBA", (WIDTH, banner_h), (10, 10, 15, 40))
        banner = Image.alpha_composite(banner, dark)

        # paste banner at top (y=0)
        banner_y = 0
        card.paste(banner, (0, banner_y), banner)

        # Title directly on banner at top - much bigger
        title_text = "VOICE STATS"
        padding_x = 80

        def fit_font_for_width(text, base_font, max_width, min_size=800):  # 10x bigger min size
            size = getattr(base_font, "size", 1600) 
            for s in range(size, min_size - 1, -20):
                try:
        ...
        # position avatar centered horizontally, overlapping banner
        avatar_x = (WIDTH - (AV_SIZE + BORDER * 2)) // 2
        avatar_y = banner_h - (AV_SIZE + BORDER * 2) // 2  # Overlap with bottom of banner
        card.paste(avatar_border, (avatar_x, avatar_y), avatar_border)

        # Username centered under avatar - 10x BIGGER
        username_text = user.name.upper()
        try:
            username_bold_font = ImageFont.truetype("arialbd.ttf", 800)  # 10x bigger
        except:
            username_bold_font = username_font
        ...
        uname_y = avatar_y + AV_SIZE + BORDER * 2 + 40  # More spacing
        
        # Red username with dark shadow
        draw.text((uname_x + 4, uname_y + 4), username_text, font=username_bold_font, fill=(20, 20, 20))
        draw.text((uname_x, uname_y), username_text, font=username_bold_font, fill=(200, 30, 30))

        # Stats area: much bigger boxes and text
        stats_top = uname_y + 200  # More spacing for bigger text
        y = stats_top
        spacing = 200  # Much bigger spacing between boxes
        box_padding = 60  # Much bigger padding
        box_left = 80  # Bigger margins
        box_right = WIDTH - 80  # Bigger margins

        # fonts for labels/values - 10x bigger
        try:
            label_big = ImageFont.truetype("arialbd.ttf", 700)  # 10x bigger
            value_big = ImageFont.truetype("arialbd.ttf", 700)  # 10x bigger
        except Exception:
            try:
                label_big = ImageFont.truetype("arial.ttf", 700)  # 10x bigger
                value_big = ImageFont.truetype("arial.ttf", 700)  # 10x bigger
            except Exception:
        ...
        def stat(label, value):
            ...
            glow_layer = glow_layer.filter(ImageFilter.GaussianBlur(radius=5))  # More blur
