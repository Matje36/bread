...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        # Retrieve Voice Client explicitly (robust check)
        vc = discord.utils.get(self.bot.voice_clients, guild=interaction.guild)

        if not vc or not vc.is_connected():
            # This block now handles the 'disconnected unexpectedly' message.
            if len(queue) > 0:
    ...
    async def play(self, interaction: discord.Interaction, url: str):
        ...
        if not vc:
            ...
            await asyncio.sleep(0.5) # FIX: Add delay after connecting
        elif vc.channel != channel:
            await vc.move_to(channel)
            await asyncio.sleep(0.5) # FIX: Add delay after moving
    ...
    async def testsound(self, interaction: discord.Interaction):
        ...
        if not vc:
            ...
            await asyncio.sleep(0.5) # FIX: Add delay
    ...
    # --- GROUP: /call (Connection Management) ---
    ...
    @call_group.command(name="leave", description="Disconnects the bot and clears the queue.")
    async def call_leave(self, interaction: discord.Interaction):
        vc = interaction.guild.voice_client
        if vc:
            # Clear the queue to prevent the next song from loading on disconnect/reconnect
            self.queues[interaction.guild_id] = deque() 
            
            # Note: Explicitly stopping the music (vc.stop()) is usually recommended 
            # for stability, but disconnecting will implicitly stop the stream.
