...
# FFmpeg Options: Crucial for streaming stability and reconnection logic
FFMPEG_OPTIONS = {
    'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
    'options': '-vn' # -vn tells ffmpeg not to look for video
}

# YDL Options: Includes cookies for YouTube IP bypass
YDL_OPTIONS = {
    ...
    'logtostderr': False,
    'no_warnings': True,
    # === CRITICAL FIX FOR 403 ERRORS ON RAILWAY ===
    # REQUIRES 'cookies.txt' file in the root directory
...
class MusicSystem(commands.Cog):
    ...
    def get_queue(self, guild_id):
        if guild_id not in self.queues:
            self.queues[guild_id] = deque() # Use deque for efficient queue handling
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        if not voice_client or not voice_client.is_connected():
            # Cleanup if bot unexpectedly disconnected
        ...
        if not queue:
            # Queue is empty, stop and clear current song state
            self.current_song.pop(guild_id, None)
            return
        ...
        def after_playing(error):
            ...
            # Schedule the next song safely back onto the event loop
        ...
        try:
            print(f"DEBUG: Attempting to play {title} using stream URL.")
            
            source = discord.FFmpegPCMAudio(url, **FFMPEG_OPTIONS)
            source = discord.PCMVolumeTransformer(source, volume=0.5)
            
            # Stop any currently playing audio before starting the new one (Safety Check)
            ...
            # If playback fails here (e.g., FFmpeg can't open stream)
            print(f"CRITICAL ERROR playing song: {e}")
            await interaction.channel.send(f"⚠️ Error playing **{title}**. Check cookies/FFmpeg. Attempting next song...")
            await self.play_next(interaction) # Try the next song

    # ---------------------------------------------------------
    # COMMAND GROUPS
    # ---------------------------------------------------------

    # Group commands to /sound [subcommand]
    sound_group = app_commands.Group(name="sound", description="Music playback commands", guild_ids=[GUILD_ID])
    # Group commands to /call [subcommand]
    ...
    async def call_leave(self, interaction: discord.Interaction):
        voice_client = interaction.guild.voice_client
        if voice_client:
            # Stop playback and clear state upon leaving
            if voice_client.is_playing():
                 voice_client.stop()
    ...
    async def sound_play(self, interaction: discord.Interaction, url: str):
        ...
        try:
            with yt_dlp.YoutubeDL(YDL_OPTIONS) as ydl:
                ...
                if 'entries' in info:
                    info = info['entries'][0] # Pick first result if search/playlist
            ...
            # This is where the 403 (cookie) error is likely caught.
            print(f"❌ YTDLP Error: {e}")
            return await interaction.followup.send(f"❌ Error fetching song info (Check if **cookies.txt** is uploaded): {e}")
    ...
    async def sound_skip(self, interaction: discord.Interaction):
        ...
        if voice_client and voice_client.is_playing():
            # .stop() triggers the 'after' callback, which calls play_next automatically
            voice_client.stop()
    ...
    async def sound_queue(self, interaction: discord.Interaction):
        ...
        desc = ""
        # Get the current song status (optional but helpful)
    ...
    async def sound_refresh(self, interaction: discord.Interaction):
        """Emergency command to clear all state and force the bot out of voice."""
