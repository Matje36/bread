...
class VoiceTracker(commands.Cog):
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        try:
            ...
            label_big = ImageFont.truetype("arialbd.ttf", 60)   # White Labels (was 50)
            value_big = ImageFont.truetype("arialbd.ttf", 60)   # Green Values (was 50)
            username_bold_font = ImageFont.truetype("arialbd.ttf", 100) # Red Username (was 80)
        except Exception:
        ...
        banner_h = 350 # Adjusted banner height (was 300)
        
        # Use ImageOps.cover to fill the banner area without stretching
        banner = ImageOps.cover(avatar.copy(), (WIDTH, banner_h), method=Image.Resampling.LANCZOS)
        ...
        # Position avatar vertically centered *in the banner*
        avatar_y = (banner_h // 2) - (avatar_full_size // 2) 
        ...
        # Position username below the banner
        uname_y = banner_h + 30
        ...
        # Increased spacing below username to account for larger font
        stats_top = uname_y + 150 
        y = stats_top
        spacing = 100 # Vertical space between boxes (was 85)
        box_padding = 15 # Padding inside the box (was 25) - closer to edge
        box_left = 40 # Margin on left
        box_right = WIDTH - 40 # Margin on right
        box_height = 80 # Fixed height for the boxes (was 65)
