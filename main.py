...
# YT-DLP Instellingen (voor snelheid en audio kwaliteit)
YTDL_OPTIONS = {
    ...
    'no_warnings': True,
    'default_search': 'auto',
    'source_address': '0.0.0.0', # Voorkomt ipv6 issues
    'http_headers': { # Helpt tegen IP-blokkades
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    }
}

# FFmpeg Instellingen (voor stabiele stream)
FFMPEG_OPTIONS = {
    'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
    'options': '-vn -loglevel quiet', # Geen video, alleen audio, stillere logs
}
...
class MusicSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # Wachtrij structuur: {guild_id: deque([url1, url2, ...])}
        self.queues = {} 
    ...
    async def play_next(self, interaction: discord.Interaction):
        """Speelt het volgende nummer in de wachtrij."""
        guild_id = interaction.guild_id
        queue = self.get_queue(guild_id)
        
        # Check of er nog nummers zijn
        if len(queue) == 0:
            await interaction.channel.send("✅ Wachtrij is leeg. Klaar met spelen.")
            return

        # Haal volgende url op
        url = queue.popleft()
        ...
        # Functie om audio source te maken (draait in executor om bot niet te blokkeren)
        loop = asyncio.get_event_loop()
        
        try:
            # 1. Info ophalen (snelle stap)
            data = await loop.run_in_executor(None, lambda: ytdl.extract_info(url, download=False))
            song_url = data.get('url')
            title = data.get('title', 'Onbekend nummer')
            
            # 2. Audio stream starten (vereist FFmpeg)
            source = discord.FFmpegPCMAudio(song_url, **FFMPEG_OPTIONS)
            
            # Callback: als liedje klaar is, check volgende
            def after_playing(error):
                if error:
                    # Dit wordt getriggerd als de stream stopt door een fout (bv. netwerk)
                    print(f"Player error: {error}")
                # Roep play_next weer aan in de bot's loop
                fut = asyncio.run_coroutine_threadsafe(self.play_next(interaction), self.bot.loop)
                try:
                    fut.result()
                except Exception as e:
                    print(f"Fout bij starten volgende nummer in thread: {e}")

            vc.play(source, after=after_playing)
            
            # Stuur update in chat
            await interaction.channel.send(f"▶️ Nu aan het spelen: **{title}**")
            
        except yt_dlp.DownloadError as e:
            fout_detail = str(e).split('\n')[0] # Pak alleen de eerste regel
            print(f"YTDL Fout: {fout_detail}")
            await interaction.channel.send(f"❌ **Downloadfout**: Kan de URL niet verwerken. Reden: `{fout_detail}`", ephemeral=False)
            # Probeer het volgende nummer als deze faalt
            await self.play_next(interaction)
        except Exception as e:
            traceback.print_exc() # Print de volledige traceback in de console logs
            # Voeg het nummer terug toe als het niet is gelukt om af te spelen
            queue.appendleft(url) 
            await interaction.channel.send(
                f"⚠️ **Algemene Fout bij Afspelen** (`{type(e).__name__}`): Het nummer is terug in de wachtrij gezet. Controleer of FFmpeg correct is geïnstalleerd op Railway.", 
                ephemeral=False
            )


    # Maak een Group aan genaamd "youtube"
    youtube_group = app_commands.Group(name="youtube", description="Muziek commando's", guild_ids=[GUILD_ID])

    @youtube_group.command(name="play", description="Speel muziek van een URL (of voeg toe aan wachtrij)")
    async def play(self, interaction: discord.Interaction, url: str):
        await interaction.response.defer()
        
        # Check voice channel
        if not interaction.user.voice:
            return await interaction.followup.send("❌ Je moet in een voice channel zitten!")
        
        channel = interaction.user.voice.channel
        vc = interaction.guild.voice_client

        # Join als de bot er nog niet is
        if not vc:
            vc = await channel.connect()
        elif vc.channel != channel:
            # Verplaats naar het kanaal van de gebruiker
        ...
        # Als er al iets speelt, zeg dat het toegevoegd is
        if vc.is_playing() or vc.is_paused():
            await interaction.followup.send(f"📝 Toegevoegd aan wachtrij: `{url}`")
        else:
            # Als er niets speelt, start direct
            await interaction.followup.send(f"🔍 Laden...")
            await self.play_next(interaction)

    @youtube_group.command(name="pause", description="Pauzeer de muziek")
    async def pause(self, interaction: discord.Interaction):
        vc = interaction.guild.voice_client
        if vc and vc.is_playing():
            vc.pause()
            await interaction.response.send_message("⏸️ Muziek gepauzeerd.")
        else:
            await interaction.response.send_message("❌ Er speelt niets om te pauzeren.", ephemeral=True)

    @youtube_group.command(name="resume", description="Hervat de muziek")
    async def resume(self, interaction: discord.Interaction):
        vc = interaction.guild.voice_client
        if vc and vc.is_paused():
            vc.resume()
            await interaction.response.send_message("▶️ Muziek hervat.")
        else:
            await interaction.response.send_message("❌ Muziek is niet gepauzeerd.", ephemeral=True)

    @youtube_group.command(name="skip", description="Sla het huidige nummer over")
    async def skip(self, interaction: discord.Interaction):
        vc = interaction.guild.voice_client
        if vc and (vc.is_playing() or vc.is_paused()):
            # Door te stoppen, triggert de 'after' functie van play_next automatisch het volgende nummer
            vc.stop() 
            await interaction.response.send_message("⏭️ Nummer overgeslagen!")
        else:
            await interaction.response.send_message("❌ Er speelt niets om over te slaan.", ephemeral=True)

    @youtube_group.command(name="stop", description="Stop muziek en disconnect")
    async def stop(self, interaction: discord.Interaction):
        vc = interaction.guild.voice_client
        if vc:
            # Wachtrij legen
            self.queues[interaction.guild_id] = deque()
            await vc.disconnect()
            await interaction.response.send_message("🛑 Gestopt en gedisconnect.")
        else:
            await interaction.response.send_message("❌ Ik ben niet verbonden.", ephemeral=True)

    @youtube_group.command(name="queue", description="Bekijk de wachtrij")
    async def queue_cmd(self, interaction: discord.Interaction):
        queue = self.get_queue(interaction.guild_id)
        if len(queue) == 0:
            return await interaction.response.send_message("De wachtrij is leeg.")
        
        msg = "**🎶 Muziek Wachtrij:**\n"
        # Laat max 10 zien om spam te voorkomen
        for i, url in enumerate(list(queue)[:10], 1):
            msg += f"`{i}. {url}`\n"
        
        if len(queue) > 10:
            msg += f"...en nog {len(queue)-10} andere nummers."
            
        await interaction.response.send_message(msg)

    # Tijdelijk commando om te testen of FFmpeg werkt
    @youtube_group.command(name="testsound", description="Test of audio werkt zonder YouTube")
    async def testsound(self, interaction: discord.Interaction):
        # Directe link naar een rechtenvrij geluidje (geen YouTube)
        url = "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"
        
        if not interaction.user.voice:
            return await interaction.response.send_message("Ga eerst in een voice channel.")
        ...
        await interaction.response.send_message("🔊 Test geluid afspelen...")
        
        # We gebruiken hier GEEN ytdl, alleen direct FFmpeg
        try:
            vc.stop() # Stop eventuele lopende muziek
            vc.play(discord.FFmpegPCMAudio(url, **FFMPEG_OPTIONS))
            await interaction.followup.send("✅ Test succesvol gestart! Als je geluid hoort, is FFmpeg correct geïnstalleerd.", ephemeral=False)
        except Exception as e:
            traceback.print_exc()
            await interaction.followup.send(f"❌ **FFmpeg Fout**: Kon de audiostream niet starten. Dit wijst 99% zeker op een ontbrekende of foutieve FFmpeg installatie. Fout: `{e}`", ephemeral=False)
...
async def main():
    await bot.add_cog(VoiceTracker(bot))
    await bot.add_cog(MusicSystem(bot)) # <--- DEZE REGEL TOEVOEGEN
