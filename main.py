...
class VoiceTracker(commands.Cog):
    ...
    async def generate_gta_card(self, user: discord.User, data):
        # Canvas Settings
        WIDTH, HEIGHT = 900, 1500  # VERHOOGDE HOOGTE
        # Modern Dark Grey background instead of solid black
        card = Image.new("RGBA", (WIDTH, HEIGHT), (25, 25, 30, 255)) 
        draw = ImageDraw.Draw(card)

        # --- Font Loading with Linux/Railway Fallbacks ---
        def load_font(name, size):
            paths = [
                name, # Local/Windows
                f"/usr/share/fonts/truetype/dejavu/{name.replace('arial', 'DejaVuSans')}", # Linux Common
                "/usr/share/fonts/TTF/Arial.ttf" # Some containers
            ]
            for p in paths:
                try:
        ...
        # Fonts (VEEL GROTERE TEKST)
        font_header_bold = load_font("arialbd.ttf", 200) # Was 110
        font_label = load_font("arialbd.ttf", 150) # Was 65
        font_value = load_font("arial.ttf", 150) # Was 45
        ...
        banner_h = 450 # VERHOOGDE BANNER HOOGTE
        banner = avatar_src.copy().resize((WIDTH, banner_h))
        banner = banner.filter(ImageFilter.GaussianBlur(radius=15))
        
        # Gradient Fade for Banner (Top to Bottom fade out)
        # Create a gradient mask
        gradient = Image.new('L', (WIDTH, banner_h), 0)
        g_draw = ImageDraw.Draw(gradient)
        # Verander de gradient om minder snel te faden, zodat de boxen onder de banner komen
        for y in range(banner_h):
            # Alpha gaat van 255 (boven) naar 100 (onder) om een donkere overlay te creëren
            alpha = int(255 * (1 - (y / banner_h)))
            g_draw.line([(0, y), (WIDTH, y)], fill=alpha)
        
        # Apply darker overlay to banner
        overlay = Image.new("RGBA", (WIDTH, banner_h), (0, 0, 0, 100))
        banner = Image.alpha_composite(banner, overlay)
        
        # Paste banner onto card (geen mask, direct plakken)
        card.paste(banner, (0, 0)) # GEEN MASK, OM TOT AAN DE RAND TE KOMEN

        # --- 2. AVATAR (Floating) ---
        av_size = 250 # Was 200
        av_x = (WIDTH - av_size) // 2
        av_y = banner_h - (av_size // 2) - 80 # Verlaag de avatar positie
        ...
        t_bbox = draw.textbbox((0,0), name_text, font=font_header_bold)
        t_w = t_bbox[2] - t_bbox[0]
        name_x = (WIDTH - t_w) // 2
        name_y = av_y + av_size + 40 # Verhoogde Y positie voor grotere tekst
        
        draw.text((name_x + 5, name_y + 5), name_text, font=font_header_bold, fill=(0, 0, 0))
        ...
        start_y = name_y + 200 # Hogere start Y voor de boxen
        item_height = 100 # Was 75
        margin_x = 50
        
        for i, (label, value) in enumerate(stats_list):
            y_pos = start_y + (i * (item_height + 20)) # Grotere afstand tussen boxen
            
            # Row Background (Semi-transparent dark grey)
            # Create a separate layer for transparency
            row_layer = Image.new("RGBA", (WIDTH, item_height), (0,0,0,0))
            row_draw = ImageDraw.Draw(row_layer)
            
            # Draw rounded rectangle equivalent
            # Fill
            row_draw.rectangle([margin_x, 0, WIDTH - margin_x, item_height], fill=(40, 40, 45, 255))
            
            # Accent Bar on Left (Green for GTA Money vibe)
            row_draw.rectangle([margin_x, 0, margin_x + 12, item_height], fill=(255, 0, 0)) # Dikkere balk
            
            # Paste the row
            card.paste(row_layer, (0, y_pos), row_layer)
            
            # Text
            # Label (Left)
            # Y-offset aangepast voor grotere tekst
            draw.text((margin_x + 40, y_pos + (item_height - font_label.getbbox(label)[3]) // 2), label, font=font_label, fill=(220, 220, 220))
            
            # Value (Right)
            val_bbox = draw.textbbox((0,0), value, font=font_value)
            val_w = val_bbox[2] - val_bbox[0]
            # Y-offset aangepast voor grotere tekst
            draw.text((WIDTH - margin_x - val_w - 40, y_pos + (item_height - font_value.getbbox(value)[3]) // 2), value, font=font_value, fill=(255, 255, 255))
