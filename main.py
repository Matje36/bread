...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        if len(queue) == 0:
            await interaction.channel.send("✅ Queue is empty. Finished playing.")
            return
        ...
        # Function to create audio source (runs in executor to prevent blocking)
        ...
        try:
            ...
            def after_playing(error):
                if error:
                    # This is triggered if the stream stops due to an error (e.g. network)
            ...
            # Try the next song if this one fails
            await self.play_next(interaction)
        except Exception as e:
            traceback.print_exc() # Print the full traceback in the console logs
            # Add the song back to the queue if it failed to play
            queue.appendleft(url) 
            
            # Specific check for FFmpeg not found error
            error_message = str(e)
            if "ffmpeg was not found" in error_message:
                 await interaction.channel.send(
                    f"⚠️ **FFmpeg Error** (`{type(e).__name__}`): The song failed to start. **FFmpeg was not found.** Please ensure `nixpacks.toml` is correctly configured on Railway and the service is redeployed.", 
                    ephemeral=False
                )
            else:
                 await interaction.channel.send(
    ...
    # Create a Group command named "youtube"
    ...
    async def play(self, interaction: discord.Interaction, url: str):
        ...
        # Join if the bot is not in VC yet
        if not vc:
        ...
        # If something is already playing, say it was added
    ...
    async def skip(self, interaction: discord.Interaction):
        ...
        if vc and (vc.is_playing() or vc.is_paused()):
            # By stopping, the 'after' function in play_next automatically triggers the next song
            vc.stop() 
    ...
    @youtube_group.command(name="stop", description="Stop music and disconnect")
    ...
    async def testsound(self, interaction: discord.Interaction):
        # Direct link to a royalty-free sound (not YouTube)
        ...
        try:
            vc.stop() # Stop any currently playing music
