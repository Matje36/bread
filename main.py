...
# ---------------------------------------------------------
# MUSIC SYSTEM (Updated with Skip, Refresh & Cookies)
# ---------------------------------------------------------

# FFmpeg opties: reconnect is cruciaal om haperingen te voorkomen
FFMPEG_OPTIONS = {
    'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
    'options': '-vn'
}

# yt-dlp opties: COOKIES ZIJN CRUCIAAL OP RAILWAY
YDL_OPTIONS = {
    'format': 'bestaudio/best',
    'noplaylist': 'True',
    'quiet': True,
    'default_search': 'auto',
    'source_address': '0.0.0.0', # Forceer IPv4
    'nocheckcertificate': True,
    'ignoreerrors': False,
    'logtostderr': False,
    'no_warnings': True,
    # Zorg dat je cookies.txt in je Railway root map uploadt!
...
class MusicSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.queues = {}       # {guild_id: [song_dict, ...]}
        self.current_song = {} # {guild_id: song_dict}
    ...
    async def play_next(self, interaction: discord.Interaction):
        """Recursieve functie die het volgende nummer speelt."""
        guild_id = interaction.guild_id
        
        # Check of de bot nog wel verbonden is
        voice_client = interaction.guild.voice_client
        if not voice_client or not voice_client.is_connected():
            # Bot is niet meer verbonden, ruim op
            self.queues[guild_id] = []
        ...
        if not queue:
            # Wachtrij is leeg
            self.current_song[guild_id] = None
            # Optioneel: leave na het laatste nummer
            # await voice_client.disconnect() 
            return

        # Haal het volgende nummer op (FIFO)
        song_info = queue.pop(0)
        self.current_song[guild_id] = song_info
        url = song_info['url']
        title = song_info['title']

        # Callback voor als het liedje klaar is (of error geeft)
        def after_playing(error):
            if error:
                print(f"⚠️ Fout tijdens afspelen: {error}")
            
            # Roep play_next opnieuw aan voor het volgende nummer
            # Dit moet threadsafe gebeuren omdat 'after' niet async is
        ...
        try:
            print(f"DEBUG: Start afspelen van {title}")
            # Audio Source creëren
            ...
            # Stuur een berichtje in de chat (optioneel, kan spammerig zijn)
            # await interaction.channel.send(f"🎶 Nu aan het spelen: **{title}**")
            
        except Exception as e:
            print(f"CRITICAL ERROR playing song: {e}")
            await interaction.channel.send(f"⚠️ Kon **{title}** niet afspelen. Ik probeer de volgende...")
            # Probeer direct de volgende als deze faalt
            await self.play_next(interaction)

    # ---------------------------------------------------------
    # COMMANDS
    # ---------------------------------------------------------
    sound_group = app_commands.Group(name="sound", description="Muziek commando's", guild_ids=[GUILD_ID])
    call_group = app_commands.Group(name="call", description="Voice channel beheer", guild_ids=[GUILD_ID])

    # --- /call COMMANDS ---

    @call_group.command(name="join", description="Forceer de bot om te joinen.")
    async def call_join(self, interaction: discord.Interaction):
        if not interaction.user.voice:
            return await interaction.response.send_message("❌ Je moet in een voice channel zitten.", ephemeral=True)
        ...
        if voice_client:
            if voice_client.channel.id != channel.id:
                ...
                await interaction.response.send_message(f"✅ Verplaatst naar **{channel.name}**!", ephemeral=True)
            else:
                await interaction.response.send_message("✅ Ik ben er al!", ephemeral=True)
        else:
            await channel.connect()
            await interaction.response.send_message(f"✅ Gejoined in **{channel.name}**!", ephemeral=True)

    @call_group.command(name="leave", description="Laat de bot de call verlaten.")
    async def call_leave(self, interaction: discord.Interaction):
        voice_client = interaction.guild.voice_client
        if voice_client:
            # Maak wachtrij leeg bij leave
            if interaction.guild.id in self.queues:
                self.queues[interaction.guild.id].clear()
            
            await voice_client.disconnect()
            await interaction.response.send_message("👋 Doei!", ephemeral=True)
        else:
            await interaction.response.send_message("❌ Ik zit in geen enkele call.", ephemeral=True)

    # --- /sound COMMANDS ---

    @sound_group.command(name="play", description="Speel muziek van YouTube.")
    @app_commands.describe(url="Link of zoekterm")
    async def sound_play(self, interaction: discord.Interaction, url: str):
        if not interaction.user.voice:
            return await interaction.response.send_message("❌ Je moet in een voice channel zitten.", ephemeral=True)

        await interaction.response.defer() # Geef tijd om te zoeken

        # Zorg dat bot verbonden is
        voice_client = interaction.guild.voice_client
        if not voice_client:
            try:
                voice_client = await interaction.user.voice.channel.connect()
            except Exception as e:
                return await interaction.followup.send(f"❌ Kon niet joinen: {e}")

        # YT-DLP Extractie
        loop = asyncio.get_running_loop()
        try:
            # Run in executor om blocking te voorkomen
            info = await loop.run_in_executor(None, lambda: yt_dlp.YoutubeDL(YDL_OPTIONS).extract_info(url, download=False))
            
            # Als het een zoekopdracht of playlist is, pak de eerste
            if 'entries' in info:
                info = info['entries'][0]
            ...
            # Als dit faalt, is het vaak de 403 error (cookies nodig)
            return await interaction.followup.send(f"❌ Fout bij ophalen muziek (Check Logs/Cookies): {e}")

        # Toevoegen aan wachtrij
        queue = self.get_queue(interaction.guild.id)
        queue.append(song)

        embed = discord.Embed(description=f"🎵 **Toegevoegd:** [{title}]({url})", color=discord.Color.blurple())
        await interaction.followup.send(embed=embed)

        # Als er niets speelt, start de motor
        if not voice_client.is_playing():
            await self.play_next(interaction)

    @sound_group.command(name="skip", description="Sla het huidige nummer over.")
    async def sound_skip(self, interaction: discord.Interaction):
        voice_client = interaction.guild.voice_client
        if voice_client and voice_client.is_playing():
            # Stop trigger de 'after' callback, die automatisch play_next aanroept
            voice_client.stop()
            await interaction.response.send_message("⏭️ Nummer overgeslagen!", ephemeral=True)
        else:
            await interaction.response.send_message("❌ Er speelt niets om over te slaan.", ephemeral=True)

    @sound_group.command(name="stop", description="Stop muziek en pauzeer wachtrij.")
    async def sound_stop(self, interaction: discord.Interaction):
        voice_client = interaction.guild.voice_client
        if voice_client and voice_client.is_playing():
            # Queue clearen zodat hij niet verder gaat
            if interaction.guild.id in self.queues:
                self.queues[interaction.guild.id].clear()
            
            voice_client.stop()
            await interaction.response.send_message("🛑 Muziek gestopt en wachtrij gewist.", ephemeral=True)
        else:
            await interaction.response.send_message("❌ Er speelt niets.", ephemeral=True)

    @sound_group.command(name="queue", description="Bekijk de wachtrij.")
    async def sound_queue(self, interaction: discord.Interaction):
        queue = self.get_queue(interaction.guild.id)
        if not queue:
            return await interaction.response.send_message("📭 De wachtrij is leeg.", ephemeral=True)

        desc = ""
        for i, song in enumerate(queue[:10]):
            desc += f"**{i+1}.** {song['title']} (aangevraagd door {song['requester']})\n"
        
        if len(queue) > 10:
            desc += f"\n*...en nog {len(queue) - 10} nummers*"

        embed = discord.Embed(title="🎶 Huidige Wachtrij", description=desc, color=discord.Color.green())
        await interaction.response.send_message(embed=embed)

    @sound_group.command(name="refresh", description="Hard reset: Kicked bot, wist wachtrij en herstart audio module.")
    async def sound_refresh(self, interaction: discord.Interaction):
        """
        Noodknop: Als de bot vastloopt.
        """
        ...
        if voice_client:
            await voice_client.disconnect()
            await interaction.response.send_message("🔄 **Refresh:** Audio systeem gereset en wachtrij gewist. Je kunt opnieuw joinen.", ephemeral=True)
        else:
            await interaction.response.send_message("🔄 **Refresh:** Wachtrij gewist (bot was niet verbonden).", ephemeral=True)
