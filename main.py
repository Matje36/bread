...
FFMPEG_OPTIONS = {
    ...
    'options': '-vn', # Geen video, alleen audio
}
...
class MusicSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # Wachtrij structuur: {guild_id: deque([song1, song2, ...])}
        self.queues = {} 
        ...
        if len(queue) > 0:
            ...
            try:
                ...
                def after_playing(error):
                    ...
                    # Roep play_next weer aan (asyncio trucje nodig omdat after_playing niet async is)
                ...
                # Stuur update in chat (optioneel, let op: interaction token kan verlopen zijn)
                # We sturen een nieuw bericht in het kanaal waar het commando vandaan kwam
                await interaction.channel.send(f"▶️ Nu aan het spelen: **{title}**")
                
            except Exception as e:
                print(f"Fout bij afspelen: {e}")
                await interaction.channel.send(f"⚠️ Er ging iets mis bij het afspelen van het volgende nummer.")
        else:
            # Wachtrij leeg
