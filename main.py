...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        try:
            ...
            stat_font = ImageFont.truetype("arial.ttf", 48)
        ...
        # Thin GTA red bar at top
        draw.rectangle([0, 0, WIDTH, 50], fill=(220, 20, 20))
        draw.rectangle([0, 48, WIDTH, 52], fill=(255, 100, 0))
        
        # Title spanning full width
        draw.text((35, 5), "VOICE STATS", font=title_font, fill=(255, 255, 255))

        # Avatar smaller with thin dark red border
        avatar_bytes = await user.display_avatar.read()
        avatar = Image.open(BytesIO(avatar_bytes)).resize((200, 200))
        
        # Create avatar with thin border
        avatar_with_border = Image.new("RGB", (210, 210), (100, 10, 10))
        avatar_with_border.paste(avatar, (5, 5))
        card.paste(avatar_with_border, (245, 70))

        # Username centered under avatar
        username_bbox = draw.textbbox((0, 0), user.name.upper(), font=stat_font)
        username_width = username_bbox[2] - username_bbox[0]
        username_x = (WIDTH - username_width) // 2
        draw.text((username_x, 295), user.name.upper(), font=stat_font, fill=(220, 20, 20))

        # Stats section with labels on left, values on right
        y = 370
        spacing = 65
        ...
        def stat(label, value):
            ...
            # Label on left
            draw.text((40, y+5), f"{label}:", font=label_font, fill=(255, 150, 0))
            # Value on right
            value_bbox = draw.textbbox((0, 0), str(value), font=stat_font)
            value_width = value_bbox[2] - value_bbox[0]
            draw.text((640 - value_width, y+5), str(value), font=stat_font, fill=(100, 255, 100))
            y += spacing
        ...
        stat("VC Partners", str(len(data["joined_with"])))
