...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        try:
            ...
            username_font = ImageFont.truetype("arial.ttf", 48)
            base_title_font = ImageFont.truetype("arial.ttf", 120)
        except Exception:
        ...
        # Background: grey with slight blue vertical gradient
        top_col = (220, 220, 225)
        bot_col = (200, 210, 235)
        ...
        bar_y0 = 0
        bar_y1 = BAR_H
        draw.rectangle([0, bar_y0, WIDTH, bar_y1], fill=(35, 35, 35, 230))
        ...
        def fit_font_for_width(text, base_font, max_width, min_size=28):
            size = getattr(base_font, "size", 120)
            # fallback simple loop reducing size until fits
            for s in range(size, min_size - 1, -2):
                try:
                    f = ImageFont.truetype(base_font.path, s) if hasattr(base_font, "path") else ImageFont.truetype("arial.ttf", s)
                except Exception:
                ...
                if w <= max_width - padding_x * 2:
                    return f, w, h
            return base_font, draw.textbbox((0, 0), text, font=base_font)[2:], 0
        ...
        # subtle shadow then white title (big)
        shadow_off = 3
        draw.text((title_x + shadow_off, title_y + shadow_off), title_text, font=title_font, fill=(20, 20, 20, 200))
        draw.text((title_x, title_y), title_text, font=title_font, fill=(245, 245, 245, 255))
        ...
        # Avatar smaller and centered, with thin dark red border
        AV_SIZE = 160
        ...
        # Username centered under avatar, big and bold
        username_text = user.name.upper()
        uname_bbox = draw.textbbox((0, 0), username_text, font=username_font)
        uname_w = uname_bbox[2] - uname_bbox[0]
        uname_x = (WIDTH - uname_w) // 2
        uname_y = avatar_y + AV_SIZE + BORDER * 2 + 10
        draw.text((uname_x + 2, uname_y + 2), username_text, font=username_font, fill=(20, 20, 20))
        draw.text((uname_x, uname_y), username_text, font=username_font, fill=(200, 30, 30))

        # Stats area: make boxes full-width and text much larger so it fits perfectly across PNG
        stats_top = uname_y + username_font.size + 18
        y = stats_top
        spacing = 86
        box_padding = 28
        box_left = 40
        ...
        # fonts for labels/values tuned so they fill boxes nicely
        try:
            label_big = ImageFont.truetype("arial.ttf", 44)
            value_big = ImageFont.truetype("arial.ttf", 62)
        except Exception:
        ...
        def stat(label, value):
            nonlocal y
            # background box that spans across image
            box_top = y
            box_bottom = y + spacing - 12
            # soft rounded-rectangle effect using lines
            draw.rectangle([box_left, box_top, box_right, box_bottom], fill=(245, 245, 248, 30), outline=(200, 50, 40, 180), width=2)
