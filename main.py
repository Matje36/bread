...
class YTAudio(discord.PCMVolumeTransformer):
    ...
    async def from_url(cls, url, *, loop=None, stream=True):
        ...
        if 'entries' in data:
            # Pak het eerste item uit een playlist
            data = data['entries'][0]

        filename = data['url'] if stream else ytdl.prepare_filename(data)
        return cls(discord.FFmpegPCMAudio(filename, **FFMPEG_OPTIONS), volume=0.5), data.get('title', 'Unknown')

# Forceer deze hele Cog om alleen op jouw specifieke GUILD_ID te werken
...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        # Haal volgende URL op (maar verwijder hem nog niet uit de queue voor het geval extract faalt)
        url = queue[0] 

        try:
            # Audio bron creëren
            # We gebruiken hier een helper methode om de event loop niet te blokkeren
            player, title = await YTAudio.from_url(url, loop=self.bot.loop, stream=True)
            
            # Als gelukt, verwijder uit queue
            queue.popleft()

            def after_playing(error):
                if error:
                    print(f"Player error: {error}")
                # Roep play_next recursief aan (threadsafe)
            ...
            # Stuur bericht naar het kanaal waar het commando vandaan kwam
            ...
            # Verwijder de foute song zodat de loop niet vastloopt
    ...
    async def skip(self, interaction: discord.Interaction):
        ...
        if vc and (vc.is_playing() or vc.is_paused()):
            vc.stop() # Dit triggert de 'after' functie in play_next, die het volgende nummer start
    ...
    @youtube_group.command(name="stop", description="Stop music and clear queue.")
...
# Setup functie blijft hetzelfde, maar zorgt dat de class correct geladen wordt
# async def setup(bot): (Deze zit al in je main block, maar zorg dat je MusicSystem hierboven gebruikt)
