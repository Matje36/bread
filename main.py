...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        # auto-scale title font size to fit across the bar — make it much bigger
        ...
        # subtle shadow then black title (big)
        shadow_off = 3
        ...
        try:
            username_font = ImageFont.truetype("arialbd.ttf", 72)
        except Exception:
            username_font = ImageFont.load_default()
        ...
        try:
            label_big = ImageFont.truetype("arialbd.ttf", 52)
            value_big = ImageFont.truetype("arialbd.ttf", 68)
        except Exception:
        ...
        def stat(label, value):
            ...
            draw.rectangle([box_left, box_top, box_right, box_bottom], fill=(0, 0, 0, 255))

            # red glow fading outward
            glow = Image.new("RGBA", (box_right - box_left, spacing), (0, 0, 0, 0))
            glow_draw = ImageDraw.Draw(glow)
            for i in range(12, 0, -1):  # fading layers
                alpha = int(80 * (i / 12))
                glow_draw.rectangle([0, 0, box_right - box_left, spacing], fill=(200, 30, 30, alpha))
                glow = glow.filter(ImageFilter.GaussianBlur(radius=i))
            card.paste(glow, (box_left, box_top), glow)
