...
# Verbeterde FFmpeg opties voor streaming stabiliteit
FFMPEG_OPTIONS = {
    'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
    'options': '-vn'
}

# Verbeterde yt-dlp opties om IP-blokkades en 403 errors te verminderen
YDL_OPTIONS = {
    ...
    'quiet': True,
    'no_warnings': True,
    # User agent spoofs om YouTube blokkades te omzeilen
...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        def after_playing(error):
            ...
            # Veilig de volgende song starten
        ...
        try:
            # We gebruiken FFmpegOpusAudio indien mogelijk, anders PCM
            # PCM is vaak stabieler met reconnect options
            source = discord.FFmpegPCMAudio(url, **FFMPEG_OPTIONS)
            source = discord.PCMVolumeTransformer(source, volume=0.5)
            
            voice_client.play(source, after=after_playing)
            print(f"DEBUG: Started playing {title}")
        except Exception as e:
            print(f"CRITICAL ERROR playing song: {e}")
            await interaction.channel.send(f"⚠️ Error playing **{title}**. Skipped.")
            await self.play_next(interaction)

    # ... (De rest van de commands /call en /sound blijven hetzelfde) ...
    # Zorg dat je de call_group en sound_group commands hieronder laat staan
    ...
    async def sound_play(self, interaction: discord.Interaction, url: str):
        ...
        # Extractie met extra foutafhandeling
