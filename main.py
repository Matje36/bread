...
# ---------------------------------------------------------
# LIVE UPDATING STOPWATCH SYSTEM
# ---------------------------------------------------------
...
async def run_stopwatch(user_id, interaction):
    """Background task that updates the stopwatch every second."""
    data = active_stopwatches[user_id]
    ...
    while True:
        ...
        # Stopwatch bestaat niet meer → stop taak
        if user_id not in active_stopwatches:
            return
        ...
        try:
            await message.edit(content=f"⏱️ **Running Stopwatch**\nTime: `{h:02d}:{m:02d}:{s:02d}`")
        except:
            pass
...
@bot.tree.command(
    ...
    description="Start a live updating stopwatch.",
    guild=discord.Object(id=GUILD_ID)
)
async def stopwatch_start(interaction: discord.Interaction):
    user_id = str(interaction.user.id)

    # Already running?
    if user_id in active_stopwatches:
        await interaction.response.send_message("⏱️ Your stopwatch is already running!", ephemeral=True)
        return

    await interaction.response.defer()

    msg = await interaction.channel.send("⏱️ **Starting stopwatch...**")

    active_stopwatches[user_id] = {
        "start": time.time(),
        "message": msg,
        "task": asyncio.create_task(run_stopwatch(user_id, interaction))
    }

    await interaction.followup.send("✅ Stopwatch started!", ephemeral=True)


@bot.tree.command(
    name="stopwatch_stop",
    description="Stop your live stopwatch.",
    guild=discord.Object(id=GUILD_ID)
)
async def stopwatch_stop(interaction: discord.Interaction):
    user_id = str(interaction.user.id)

    if user_id not in active_stopwatches:
        await interaction.response.send_message("❌ You have no active stopwatch!", ephemeral=True)
        return
    ...
    # Remove entry so update loop ends
    del active_stopwatches[user_id]
    ...
    await message.edit(
        content=f"⏱️ **Stopwatch Stopped**\nFinal Time: `{h:02d}:{m:02d}:{s:02d}`"
    )

    await interaction.followup.send("🛑 Stopwatch stopped!", ephemeral=True)


@bot.tree.command(
    name="stopwatch_reset",
    description="Reset and delete your active stopwatch.",
    guild=discord.Object(id=GUILD_ID)
)
async def stopwatch_reset(interaction: discord.Interaction):
    user_id = str(interaction.user.id)

    if user_id not in active_stopwatches:
        await interaction.response.send_message("❌ You don't have a running stopwatch!", ephemeral=True)
        return
    ...
    await message.edit(content="🔄 Stopwatch **reset**.")
    await interaction.response.send_message("✅ Stopwatch has been reset.", ephemeral=True)
