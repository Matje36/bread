...
class MusicSystem(commands.Cog):
    ...
    async def play_next(self, interaction):
        ...
        try:
            ...
            await interaction.channel.send(f"❌ Could not process: `{e}`")
            return
        ...
        try:
            ...
            await interaction.channel.send(f"❌ FFmpeg Playback Error: `{e}`")

    # ----------------------------------------------------
    # YOUTUBE COMMAND GROUP (ONLY PARENT HAS GUILD SET)
    # ----------------------------------------------------
    youtube_group = app_commands.Group(
        name="youtube",
        description="YouTube music commands"
    )
    youtube_group.guild_ids = [GUILD_ID]  # ✔ FIXED: GUILD RESTRICTED HERE ONLY
    ...
    # /youtube testsound
    # ----------------------------------------------------
    @youtube_group.command(name="testsound", description="Test if FFmpeg audio works.")
    async def testsound(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("🔊 Test sound playing — FFmpeg works!")
        except Exception as e:
