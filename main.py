...
class BotClient(commands.Bot):
    ...
    async def setup_hook(self):
        # Hier laden we de MusicSystem class
        await self.add_cog(MusicSystem(self))
        # En eventueel andere cogs zoals VoiceTracker
        await self.add_cog(VoiceTracker(self))
        
        # Kopieer en sync commands
        self.tree.copy_global_to(guild=self.target_guild)
        await self.tree.sync(guild=self.target_guild)
        print("✅ Setup hook finished & Commands synced!")
...
# =================================================================================================
#  YOUTUBE MUSIC SYSTEM (STABLE & GUILD LOCKED)
# =================================================================================================

# Zorg dat je deze imports bovenaan je bestand hebt staan:
# import yt_dlp
# from collections import deque
# import asyncio
# import discord
...
# Configuratie voor yt-dlp (zorgt voor stabiliteit en snelheid)
YTDL_OPTIONS = {
...
# Configuratie voor FFmpeg (voorkomt dat de stream stopt bij packet loss)
FFMPEG_OPTIONS = {
...
class YTAudio(discord.PCMVolumeTransformer):
    """
    Helper class om audio streams te beheren zonder de bot te blokkeren.
    """
    ...
    async def from_url(cls, url, *, loop=None, stream=True):
        ...
        # Voer de extractie uit in een executor om blocking te voorkomen
        data = await loop.run_in_executor(None, lambda: ytdl.extract_info(url, download=not stream))

        if 'entries' in data:
            # Als het een playlist/search result is, pak de eerste
            data = data['entries'][0]

        filename = data['url'] if stream else ytdl.prepare_filename(data)
        return cls(discord.FFmpegPCMAudio(filename, **FFMPEG_OPTIONS), volume=0.5), data.get('title', 'Unknown Title')
...
# BELANGRIJK: Deze decorator zorgt dat commands ALLEEN op jouw GUILD_ID laden.
# Dit fixt het probleem dat commands niet verschijnen.
@app_commands.guilds(discord.Object(id=GUILD_ID))
class MusicSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.queues = {}  # Opslag per guild: {guild_id: deque()}
    ...
    async def play_next(self, interaction: discord.Interaction):
        """
        Interne functie die de queue afhandelt.
        Wordt recursief aangeroepen door de 'after' callback.
        """
        ...
        if len(queue) == 0:
            return  # Queue is leeg, niets doen

        # Pak de URL (maar verwijder nog niet, voor het geval extractie faalt)
        url = queue[0]

        try:
            # Haal audio bron op (non-blocking)
            player, title = await YTAudio.from_url(url, loop=self.bot.loop, stream=True)
            
            # Als extractie lukt, verwijder uit queue
            queue.popleft()

            # Callback functie voor wanneer het liedje klaar is
            def after_playing(error):
                if error:
                    print(f"Player error: {error}")
                
                # Roep play_next opnieuw aan voor het volgende nummer
                # We gebruiken run_coroutine_threadsafe omdat 'after' niet async is
            ...
            # Stuur bericht (als het kanaal nog bestaat)
            ...
            # Verwijder de foute song om oneindige loop te voorkomen
            if queue and queue[0] == url:
                queue.popleft()
            
            # Probeer het volgende nummer
            await self.play_next(interaction)

    # Groep aanmaken voor /youtube [subcommand]
    youtube_group = app_commands.Group(name="youtube", description="YouTube music commands")

    @youtube_group.command(name="play", description="Play a YouTube song or add it to the queue.")
    async def play(self, interaction: discord.Interaction, query: str):
        await interaction.response.defer() # Geef de bot tijd om te laden
        ...
        # Als er al iets speelt, alleen melden dat het in de queue staat
        if vc.is_playing() or vc.is_paused():
            return await interaction.followup.send(f"➕ Added to queue: `{query}`")

        # Zo niet, start afspelen
    ...
    async def skip(self, interaction: discord.Interaction):
        ...
        if vc and (vc.is_playing() or vc.is_paused()):
            vc.stop() # Dit triggert automatisch 'after_playing' -> volgende nummer
    ...
    async def pause(self, interaction: discord.Interaction):
        ...
        if vc and vc.is_playing():
            vc.pause()
            return await interaction.response.send_message("⏸️ Paused.")
        await interaction.response.send_message("❌ Nothing is playing.", ephemeral=True)

    @youtube_group.command(name="resume", description="Resume music playback.")
    async def resume(self, interaction: discord.Interaction):
        vc = interaction.guild.voice_client
        if vc and vc.is_paused():
            vc.resume()
            return await interaction.response.send_message("▶️ Resumed.")
        await interaction.response.send_message("❌ Music is not paused.", ephemeral=True)

    @youtube_group.command(name="queue", description="Show the music queue.")
    async def queue_cmd(self, interaction: discord.Interaction):
        queue = self.get_queue(interaction.guild.id)
        if not queue:
            return await interaction.response.send_message("📭 Queue is empty.")

        # Laat max 10 regels zien
        lines = [f"{i+1}. `{url}`" for i, url in enumerate(list(queue)[:10])]
        msg = "**🎶 Current Queue:**\n" + "\n".join(lines)
    ...
    @youtube_group.command(name="stop", description="Clear queue and disconnect.")
    ...
    @youtube_group.command(name="testsound", description="Test FFmpeg audio.")
    async def testsound(self, interaction: discord.Interaction):
        ...
        # Simpele MP3 test URL
        test_url = "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"
        try:
            vc.stop()
            vc.play(discord.FFmpegPCMAudio(test_url, **FFMPEG_OPTIONS))
            await interaction.followup.send("🔊 Playing test sound...")
        except Exception as e:
