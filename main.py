...
class VoiceTracker(commands.Cog):
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        WIDTH = 900
        HEIGHT = 1100 # Increased height for larger fonts/boxes
        ...
        try:
            ...
            label_big = ImageFont.truetype("arialbd.ttf", 50)   # White Labels
            value_big = ImageFont.truetype("arialbd.ttf", 50)   # Green Values
            username_bold_font = ImageFont.truetype("arialbd.ttf", 80) # Red Username
        except Exception:
        ...
        banner_h = 300 # Adjusted banner height
        ...
        # --- TITLE REMOVED ---
        # (Title code block was here)
        # (Separator line code block was here)

        # Avatar placement: Centered, overlapping the banner
        AV_SIZE = 180 # Increased avatar size
        avatar_small = avatar.resize((AV_SIZE, AV_SIZE))
        mask = Image.new("L", (AV_SIZE, AV_SIZE), 0)
        mask_draw = ImageDraw.Draw(mask)
        mask_draw.ellipse((0, 0, AV_SIZE, AV_SIZE), fill=255)

        BORDER = 10 # Increased border size
        ...
        # Position relative to bottom of banner
        avatar_y = banner_h - (avatar_full_size // 2) - 10 
        card.paste(avatar_border, (avatar_x, avatar_y), avatar_border)

        # --- Username centered under avatar (Increased font) ---
        ...
        uname_y = avatar_y + avatar_full_size + 20 # Just below the avatar border
        ...
        stats_top = uname_y + 120 # Starting position for the first box (Increased spacing)
        y = stats_top
        spacing = 85 # Vertical space between boxes (Box height + 20)
        box_padding = 25 # Padding inside the box (text to box border)
        box_left = 40 # Margin on left
        box_right = WIDTH - 40 # Margin on right
        box_height = 65 # Fixed height for the boxes
        ...
        if y > HEIGHT:
            # If stats overflow, we need a taller canvas (this shouldn't happen with 1100px)
