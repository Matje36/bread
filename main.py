...
async def oxo(interaction: discord.Interaction, opponent: discord.User = None):
    if opponent and opponent != interaction.user:
        ...
        class ChoiceView(discord.ui.View):
            def __init__(self):
                super().__init__(timeout=None) # Note: timeout=None can be risky if not managed
...
class VoiceTracker(commands.Cog):
    ...
    # ------------------------------------------------------------------
    #  IMAGE GENERATOR (OPTIMIZED DISCORD CARD V3)
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    #  IMAGE GENERATOR (OPTIMIZED DISCORD CARD V4 - GTA/MODERN STYLE)
    # ------------------------------------------------------------------
    async def generate_gta_card(self, user: discord.User, data):
        # 1. Setup Canvas
        W, H = 800, 750  # Iets compacter en breder voor betere leesbaarheid
        # Kleurenpalet (GTA / Dark Mode stijl)
        BG_COLOR = (25, 25, 25)
        CARD_BG = (40, 40, 40)
        TEXT_WHITE = (240, 240, 240)
        TEXT_GRAY = (180, 180, 180)
        ACCENT_RED = (220, 50, 50) # GTA Rood

        card = Image.new("RGBA", (W, H), BG_COLOR)
        draw = ImageDraw.Draw(card)

        # 2. Fonts laden (Fallback systeem zodat het werkt op Windows/Linux)
        ...
        # Probeer een dikkeredrukte font te vinden
        font_paths = ["arialbd.ttf", "impact.ttf", "segoeui.ttf", "DejaVuSans-Bold.ttf", "robotobold.ttf"]
        font_bold = ImageFont.load_default()
        font_reg = ImageFont.load_default()
        
        for p in font_paths:
            try:
                font_bold = ImageFont.truetype(p, 42) # Grote tekst (Values)
                font_reg = ImageFont.truetype(p, 24)  # Kleine tekst (Labels)
                font_name = ImageFont.truetype(p, 60) # Username
                break
            except OSError:
                continue
        ...
        try:
            ...
            # Fallback als avatar faalt
            avatar = Image.new("RGBA", (200, 200), (100, 100, 100))

        BANNER_H = 220
        banner = avatar.copy().resize((W, BANNER_H))
        banner = banner.filter(ImageFilter.GaussianBlur(radius=6))
        
        # Dark overlay over banner
        overlay = Image.new("RGBA", (W, BANNER_H), (0, 0, 0, 100))
        banner = Image.alpha_composite(banner, overlay)
        card.paste(banner, (0, 0))

        # 4. Avatar (Cirkel met rode rand, overlapt banner)
        AV_SIZE = 160
        AV_X = (W - AV_SIZE) // 2
        ...
        av_img = avatar.resize((AV_SIZE, AV_SIZE))
        
        # Rode rand tekenen
        BORDER = 6
        draw.ellipse((AV_X - BORDER, AV_Y - BORDER, AV_X + AV_SIZE + BORDER, AV_Y + AV_SIZE + BORDER), fill=ACCENT_RED)
        card.paste(av_img, (AV_X, AV_Y), mask)

        # 5. Username
        name_text = user.name.upper()
        # Bereken tekst breedte (compatible met oude en nieuwe PIL)
        bbox = draw.textbbox((0, 0), name_text, font=font_name)
        text_w = bbox[2] - bbox[0]
        
        NAME_Y = AV_Y + AV_SIZE + 20
        # Schaduw
        draw.text(((W - text_w) / 2 + 2, NAME_Y + 2), name_text, font=font_name, fill=(0,0,0))
        # Tekst Rood
        draw.text(((W - text_w) / 2, NAME_Y), name_text, font=font_name, fill=ACCENT_RED)

        # 6. Statistieken Grid (2 kolommen, 3 rijen)
        # Helper functie om tijd mooi te formatten
        def fmt(seconds):
            h = int(seconds // 3600)
        ...
        stats_list = [
            ...
            ("MUTE COUNT", str(data.get("mute_count", 0))),
            ("DEAF COUNT", str(data.get("deaf_count", 0))),
        ]
        ...
        # Grid loop
        current_x = (W - (2 * BOX_W + GAP_X)) // 2 # Centreer het blok
        current_y = START_Y
        ...
        for i, (label, value) in enumerate(stats_list):
            ...
            x = current_x + col * (BOX_W + GAP_X)
            y = current_y + row * (BOX_H + GAP_Y)

            # Teken vak (Rounded rectangle style)
            # Oudere PIL versies hebben geen rounded_rectangle, dus we doen een normale rectangle met styling
            draw.rectangle([x, y, x + BOX_W, y + BOX_H], fill=CARD_BG, outline=None)
            
            # Klein rood accent lijntje links
            draw.rectangle([x, y + 15, x + 5, y + BOX_H - 15], fill=ACCENT_RED)

            # Label (Title)
            draw.text((x + 20, y + 15), label, font=font_reg, fill=TEXT_GRAY)

            # Value (Data)
            val_bbox = draw.textbbox((0,0), value, font=font_bold)
            val_h = val_bbox[3] - val_bbox[1]
            # Plaats value iets lager
            draw.text((x + 20, y + 45), value, font=font_bold, fill=TEXT_WHITE)
