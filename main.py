...
class VoiceTracker(commands.Cog):
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        WIDTH = 1200
        # Increased HEIGHT to ensure all boxes fit, starting lower
        HEIGHT = 2200 
        card = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 255)) # Black background
        draw = ImageDraw.Draw(card)

        # Fonts (Loading with fallback, sizes adjusted for new dimensions)
        try:
            # Fonts used for stats/labels/username are now bigger, adjusted for overall image size
            label_big = ImageFont.truetype("arialbd.ttf", 90) # White Labels
            value_big = ImageFont.truetype("arialbd.ttf", 90) # Green Values
            username_bold_font = ImageFont.truetype("arialbd.ttf", 150) # Red Username
        except Exception:
        ...
        # Banner created from avatar: wide, blurred, darkened, touches top (y=0)
        banner_h = 400
        banner = avatar.copy().resize((WIDTH, banner_h))
        # Apply blur as requested
        banner = banner.filter(ImageFilter.GaussianBlur(radius=8)) # Increased blur radius

        # dark overlay
        dark = Image.new("RGBA", (WIDTH, banner_h), (10, 10, 15, 80)) # Slightly darker overlay
        ...
        # --- Avatar placement: Centered in the middle of the banner ---
        AV_SIZE = 300
        ...
        BORDER = 15 # Border size for the circle
        ...
        # Calculate position to center avatar in the banner area
        avatar_full_size = AV_SIZE + BORDER * 2
        avatar_x = (WIDTH - avatar_full_size) // 2
        # Center Y: (banner_y + banner_h) / 2 - avatar_full_size / 2
        avatar_y = banner_y + (banner_h // 2) - (avatar_full_size // 2)
        card.paste(avatar_border, (avatar_x, avatar_y), avatar_border)

        # --- Username centered under avatar, bigger red letters ---
        ...
        # Y below the avatar, with some spacing
        uname_y = banner_y + banner_h + 30 # Place immediately below the banner
        
        # Red username with dark shadow
        draw.text((uname_x + 4, uname_y + 4), username_text, font=username_bold_font, fill=(20, 20, 20)) # Shadow
        draw.text((uname_x, uname_y), username_text, font=username_bold_font, fill=(200, 30, 30)) # Red

        # --- Stats area: bigger boxes, text closer to edge ---
        stats_top = uname_y + 200 # Starting position for the first box
        y = stats_top
        spacing = 150 # Spacing between boxes (height of box + gap)
        box_padding = 40 # Padding inside the box (distance from text to box border)
        box_left = 60 # Bigger margins
        box_right = WIDTH - 60 # Bigger margins
        box_height = 120 # Fixed height for the boxes (spacing - gap)
        ...
        def stat(label, value):
            ...
            # Create red glow effect - same as before
            glow_layer = Image.new("RGBA", card.size, (0, 0, 0, 0))
            glow_draw = ImageDraw.Draw(glow_layer)
            
            # Increased glow radius and alpha for a better effect with larger box
            ...
            lbl_font_size = label_big.size if hasattr(label_big, "size") else 90
            ...
            # Aligned to the right, with box_padding
