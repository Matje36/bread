...
# ---------------------------------------------------------
# MUSIC SYSTEM CONSTANTS (Ensure 'cookies.txt' is uploaded)
# ---------------------------------------------------------
...
YDL_OPTIONS = {
    ...
    'logtostderr': False,
    'no_warnings': True,
    'cookiefile': 'cookies.txt', # CRITICAL: Reads cookies from uploaded file
...
class MusicSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.queues = {}       # {guild_id: deque([song_dict, ...])}
    ...
    async def play_next(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.channel.send(f"⚠️ Error playing **{title}**. Skipped. (Check logs for stream URL issue)")
    ...
    async def sound_play(self, interaction: discord.Interaction, url: str):
        ...
        # --- ROBUST CONNECTION LOGIC (Handles stale state) ---
        if voice_client and not voice_client.is_connected():
            try:
                await voice_client.disconnect(force=True)
                voice_client = await target_channel.connect()
            except Exception:
                # Fall through to the next check if forced disconnect failed
                voice_client = None 
        ...
        try:
            ...
            # This handles yt-dlp internal errors, often due to network issues (cookies)
    ...
    async def sound_queue(self, interaction: discord.Interaction):
        queue = self.get_queue(interaction.guild.id)
        # ... (queue formatting remains the same) ...
    ...
    async def sound_refresh(self, interaction: discord.Interaction):
        ...
        if voice_client:
            if voice_client.is_playing():
                voice_client.stop()
            await voice_client.disconnect(force=True) # Forceful disconnect
