...
#event lisent 
    ...
    async def generate_gta_card(self, user: discord.User, data):
        ...
        for y in range(HEIGHT):
            ...
            g = int(15)
            b = int(15 + (y / HEIGHT) * 30)
            draw.rectangle([0, y, WIDTH, y+1], fill=(r, g, b))

        # GTA red bar at top with glow effect
        draw.rectangle([0, 0, WIDTH, 140], fill=(220, 20, 20))
        draw.rectangle([0, 135, WIDTH, 145], fill=(255, 100, 0))  # glow outline
        
        # Title with shadow
        draw.text((42, 32), "VOICE STATS", font=title_font, fill=(40, 0, 0))  # shadow
        draw.text((40, 30), "VOICE STATS", font=title_font, fill=(255, 255, 255))

        # Decorative lines
        draw.rectangle([30, 145, 670, 148], fill=(220, 20, 20))
        draw.rectangle([30, 148, 670, 150], fill=(255, 150, 0))
        ...
        avatar_with_border = Image.new("RGB", (370, 370), (220, 20, 20))
        avatar_with_border.paste(avatar, (10, 10))
        card.paste(avatar_with_border, (165, 160))

        # Username with styling
        draw.text((32, 525), user.name.upper(), font=title_font, fill=(220, 20, 20))
        ...
        def stat(label, value):
            nonlocal y
            # Background box for each stat
            draw.rectangle([20, y-5, 680, y+55], outline=(220, 20, 20), width=2)
            draw.rectangle([22, y-3, 678, y+53], fill=(30, 30, 40))
            
            # Label and value
            draw.text((40, y+5), f"{label}", font=stat_font, fill=(255, 150, 0))
            draw.text((40, y+32), f"{value}", font=stat_font, fill=(100, 255, 100))
            y += spacing
        ...
        stat("Times Joined", str(data["joins"]))
        stat("Avg Duration", fmt(data["avg_duration"]))
        stat("Mute Count", str(data["mute_count"]))
        stat("Deaf Count", str(data["deaf_count"]))
        stat("Talking Time", fmt(data["talk_time"]))
        stat("Unique VC Partners", str(len(data["joined_with"])))

        # Bottom GTA-style footer
        draw.rectangle([0, 1050, WIDTH, 1100], fill=(220, 20, 20))
        draw.rectangle([0, 1045, WIDTH, 1050], fill=(255, 150, 0))
        
        footer_text = f"VOICE CARD • {user.name.upper()}"
        draw.text((50, 1060), footer_text, font=stat_font, fill=(255, 255, 255))
